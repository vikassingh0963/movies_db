...
from pyspark.sql.functions import (
    to_date, substring, year, col, when, lit
)
...
hdfs_partition_path = f"/Transformed_actor/{partition_name}"
...
df = (
    ...
    .option("delimiter", ",")
    .csv("hdfs://localhost:9000/data_output/actor/actor_20250907.csv")
)
...
df = df.withColumn(
    "file_date",
    to_date(
        substring(col("file_name"), 7, 8),   # extract 20250907
        "yyyyMMdd"
    )
)
...
# 3️ Get unique filename value
#file_name_value = df.select("partition_name").first()[0]   # actor_20250907.csv
 
# 4️ Write to HDFS under folder named with that file_name
output_path = f"hdfs://localhost:9000/Transformed_actor/{partition_name}"
 
df.write.mode("overwrite").option("header", True).csv(output_path)
 
# 4. Save rows to HDFS (partitioned by file_date)
#df.write.mode("overwrite").option("header", True).partitionBy("file_name").csv(
#    "hdfs://localhost:9000/Transformed_actor/"
#)
