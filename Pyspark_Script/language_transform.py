...
output_path = f"hdfs://localhost:9000/Transformed_language/{partition_name}"
 
df.write.mode("overwrite").option("header", True).csv(output_path)
#df.write.mode("overwrite").option("header", True).partitionBy("file_date").csv( "hdfs://localhost:9000/Transformed_language/")
